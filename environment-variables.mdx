...
| CB_BULLMQ_USERNAME <br /> `required` | `chartbrew` | The username used for HTTP Basic Auth on the BullMQ admin dashboard at `/apps/queues` |
| CB_BULLMQ_PASSWORD <br /> `required` | A password will be generated for you during the first run | The password used for HTTP Basic Auth on the BullMQ admin dashboard at `/apps/queues`. Set this explicitly when running Chartbrew with Docker or another environment-based deployment |
| CB_OPENAI_API_KEY <br /> CB_OPENAI_API_KEY_DEV | No default | The API key for the OpenAI API |
| CB_OPENAI_MODEL <br /> CB_OPENAI_MODEL_DEV | `gpt-5.4-mini` | The model to use for the OpenAI API |
...
| CB_DATA_API_ENABLED | `true` | Set to `false` to disable all versioned Data API routes |
| CB_DATA_API_RATE_LIMIT_MAX | `60` | Requests allowed per API key each minute |
| CB_DATA_API_AUTH_RATE_LIMIT_MAX | `30` | Failed authentication attempts allowed per IP each minute |
| CB_DATA_API_MAX_REQUEST_BYTES | `65536` | Largest accepted Data API request body, in bytes |
| CB_DATA_API_MAX_RESPONSE_BYTES | `5242880` | Largest complete Data API response, in bytes |
| CB_DATA_API_MAX_EXECUTION_MS | `30000` | Maximum Data API execution time, in milliseconds |
| CB_DATA_API_MAX_FILTERS | `50` | Maximum filters in one request |
| CB_DATA_API_MAX_VARIABLES | `100` | Maximum variables in one request |
| CB_DATA_API_MAX_VALUE_DEPTH | `10` | Maximum nesting depth for variable and filter values |
| CB_DATA_API_MAX_STRING_BYTES | `8192` | Maximum size of one input string, in bytes |
...
| CB_SLACK_SIGNING_SECRET | No default | The Slack app signing secret, used only by the server app. Copy it exactly from **Basic Information** → **App Credentials** in Slack. It is typically a 32-character hexadecimal string; do not add a `v0=` prefix, hash it, or Base64-encode it. |
| CB_ALLOW_PRIVATE_NETWORK_CALLS | `false` | `false` - API calls to private networks are not allowed <br /><br />`true` - Allow calls to private networks |
